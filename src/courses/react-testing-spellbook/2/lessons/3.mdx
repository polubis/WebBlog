...
duration: 5
description: We'll write first unit test for newly created React component.
---

<Section>
  <XL>Understanding tests for React component</XL>
  <M>
    Let's create <B>Button</B> component which will be fully customizable. We'll
    use approach called <B>types first</B> - first interfaces, implementation
    later:{" "}
  </M>
  <Snippet
  ...
  />
  <M>
    The method <B>getByText</B> allows you to check whether the text node is
    inside rendered content. This method is using <B>expect</B> inside. If the
    element is not rendered - the test will not pass (a slight shortening).
  </M>
  <M>
...
<Section>
  ...
  <Snippet>{`it('[FRAGILE] assigns class names by properties')`}</Snippet>
  <Hint hasBg>
