...
<Prelude>
  ...
  <M>
    To understand how <B>this</B> works, we must first locate the the problem,
    then solve it, and at the very end come to some conclusions.
  </M>
  <M>
    In that case, I have no other choice than to invite you to a lecture in
    which we will explain everything necessary and get to the conclusion on how
    to avoid the use of <B>this</B> keyword.
  </M>
</Prelude>
...
`}</Snippet>
  ...
  <M>
    Well, but how! This was supposed to point to instance of the <B>class</B>!
    Well, and it does point. Unfortunately not to the class. In this case, it
    points to <B>undefined</B>.
  </M>
  <M>
    <B>What?</B>
  </M>
...
<Section>
  <XL>What is lexical context?</XL>
  <M>
    In lexical context, the <B>scope</B> is determined by where the variable is
...
`}</Snippet>
  ...
  <M>
    ...
    in which our code is running. In the case of a browser it is a <B>window</B>,
    and in the case of <B>Node.js</B> it may be a <B>module</B>.
  </M>
  <M>
...
<Section>
  ...
  <M>
    If we create an object using <B>new</B>, the <B>this</B> will have the value
    of the created object. Therefore, in the following code, we have no
    problems.
  </M>
  <Snippet>{`
"use strict"
...
`}</Snippet>
  <M>
    During inheritance the <B>this</B> value of parent class is the same as
    child class. Thank goodness at least here a relief...
  </M>
  <Hint hasBg>
...
`}</Snippet>
  <M>
    ...
    class, not an instance of an object - its not there yet and will not be.
...
Most functions allow you to pass something described as <B>thisArg</B>. In the above example, we called the <B>Bike</B> function using <B>call</B>. Next we passed an <B>obj</B> object as <B>this</B>.
  ...
  <M>
 Later we did the same with <B>apply</B>, which allows to call functions with multiple arguments. In other words, we controlled binding of <B>this</B>.
  </M>
...
<Summary>
  <M>
    ...
    recommend always using <B>arrow functions</B> or simply methods such as{" "}
  ...
  <M>
    ...
    sooner or later you need to understand thoroughly in order to write code
    consciously and not expose yourself to numerous problems.
  </M>
  <M>
