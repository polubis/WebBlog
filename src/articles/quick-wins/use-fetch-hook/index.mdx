...
tags: code improvements,patterns
description: The process of creating a useForm hook to handle asynchronous operations sprinkled with a dose of real-life examples.
readTime: 9
title: useFetch hook
---
...
<Prelude>
  ...
  <M>
    So let's jump into <B>useForm</B> hook creation process and see how it
...
<Section>
  ...
  <M>
    It will be a small app for loading users from free API. You will be able to
    load users and display their avatars. Also, there will be an option to
...
<Section>
  ...
  <M>
    The first attempt should work. It works indeed but the used approach is
    dirty like a mug.
  </M>
  <Code
  ...
  <List items="Flags - easy to add a bug, A lot to test, It's hard to determine what this code does on first look, Lack of single responsibility, You can access data property when it's not ready to be accessed, Repetitive approach and sensitive to human mistake" />
...
<Section>
  ...
  <Code
    description="Control abort"
    src="https://raw.githubusercontent.com/polubis/useFetch-hook/main/snippets/abort-controller.tsx"
  />
  <M>
    In our case, we call abort all time when the <B>useEffect</B> function is
    executed.
  </M>
  <M>Also it's good to cancel:</M>
  <List items="Duplicated requests, When a user leaves the page, When components unmount" />
  <M>
...
<Section>
  ...
  <M>
    ...
    can create component for handling that.
  </M>
  <Code
...
<Summary>
  ...
  <M>
    ...
    your code base and dealing with not needed requests. I think right now you
    can boost your work a little bit.
  </M>
  <M>
