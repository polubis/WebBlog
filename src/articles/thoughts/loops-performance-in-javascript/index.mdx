...
<Section>
  ...
  <M>
    ...
    something). Used abstractions like <B>forEach</B> and <B>reduce</B> adds and
    removes anonymous functions from the call stack at each iteration, so this
    probably accounted for their inferior performance compared to the
    <B>while</B> or <B>for</B>.
  </M>
  <Hint hasBg>
    Nothing is free... While gaining in code readability, we are losing in
    performance.
  </Hint>
  <Hint hasBg>
    Such time comparisons will work much better for two different solutions
    (algorithms).
  </Hint>
</Section>
