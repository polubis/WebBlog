...
<Prelude label="Słów kilka na temat komponentów">
  <M>
  ...
  <M>
    ...
    bez żadnych dodatowych zmian - w zasadzie <B>ctrl+c</B> i <B>ctrl+v</B> i
    działa.{" "}
  </M>
  <Hint hasBg>
...
<Section>
  <XL>
    Przykład komponetu, który jest powiązany z domeną aplikacji i jest źle
    zaprojektowany
  </XL>
  <M>
...
<Section>
  <XL>Projektujemy reużywalny i rozszeżalny komponent List</XL>
  <M>
  ...
  <M>
    ...
    wskazać kształ modelu na którym będzie on operował. Teraz jeżeli będziemy
...
<Section>
  ...
  <M>
    ...
    implementacje spełniająć zawarty "kontrakt" z <B>TypeScript</B>. Kod jest
...
`}</Code>
  <M>
    Ostatnia zmiana i mamy to! Musimy wkorzystać dwa operatory: <B>spread</B>{" "}
