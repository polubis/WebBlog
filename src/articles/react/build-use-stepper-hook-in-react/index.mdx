...
description: Let's design and build a hook for handling steps logic in an easy and scalable way.
readTime: 7
...
<Section>
  <XL>1. The usage of useStepper</XL>
    <M>For a start, look at how we will use our hook. Pay attention to the hints that appear thanks to the TypeScript.</M>
  <Img description="See how it looks in the IDE" src="Type safety and protection from passing not allowed props" src="https://raw.githubusercontent.com/polubis/Snippets/main/useStepperDemo.gif" />
...
const RegisterForm = () => {
  ...
  <M>Then we used <B>as const</B> assertion, which makes <B>readonly tuple</B>.</M>
  ...
  <M>The <B>is()</B> function checks if the current step matches the passed key. If it does, there is a <B>type-guard</B> underneath, which marks <B>Component</B> as the one that match the key.</M>
  <M>Therefore, later when we want to pass on not allowed properties to component, we'll be blocked from doing that.</M>
 </Section>
...
<Snippet description='This interface describes a single step - see the demo above'>
...
interface Step {
  ...
  <M>We used the notation <B>...any[]</B> because our <B>Component</B> function will take any number of arguments, not just a single props object (we do not want to limit developers).</M>
    ...
    <M>Now let's define what our future <B>useStepper</B> must receive as parameters. The first will be <B>the key</B> of the step we want to start with, and the second will be <B>a list of steps</B>.</M>
    
    <Snippet description='This is a tuple!'>
...
`}
   ...
   <M><B>What will the hook return?</B>
    ...
    <Snippet description='The tuple again'>
...
<M><B>So what does UseStepperActions look like?</B></M>
    <Snippet description='The set of functions we will use'>
...
<M>Only the first function needs to be discussed. An example of its use is: </M>
    <Snippet description='This is how the is() function will be used'>
  {`
  if (is(step, '1')) {
...
<M>Now let's write two simple tests. The first will check if the assigned initial step has been taken into account, and the second will verify the execution of <B>set()</B> function.</M>
       <Snippet added={[[4,21]]}>
  {`
describe('Step can be changed when', () => {
...
<M>Thanks to previously created types and tests - implementation was child's play. All we had to do was push the keyboard buttons, and <B>TypeScript</B> with power of <B>TDD</B>, guided us by the hand. Tests are green ✅.</M>
  <Hint hasBg>You have whole implementation of hook in file under <A href="https://github.com/polubis/Dream-stack-for-React-dev/blob/main/system/libs/figa-hooks/src/lib/use-stepper/use-stepper.tsx" outside>this link</A>.</Hint>
</Section>
  
<Section>
<XL>5. We discuss the is() function</XL>
<M>There is one more thing left to discuss - the <B>is()</B> function. We'll skip others because <B>first, last, previous, next</B> are not worth to discuss - they are not fancy.</M>
    <Snippet description='What the heck is this syntax.'>
  {`
...
<M>This function is not "indispensable" - rather fancy syntatic sugar.</M>
  <Hint hasBg>You have whole implementation of hook in file under <A href="https://github.com/polubis/Dream-stack-for-React-dev/blob/main/system/libs/figa-hooks/src/lib/use-stepper/use-stepper.tsx" outside>this link</A>.</Hint>
</Section>
  ...
  <Summary>
  <M>Now you know how to handle the steps on the user interface in an interesting way. The created solution protects against many basic mistakes that can be made when implementing such functionality.</M>
    <M>
