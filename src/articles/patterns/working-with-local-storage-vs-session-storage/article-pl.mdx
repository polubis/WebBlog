...
<Prelude label="Wstęp">
  ...
  <M>
    Dziś stworzymy mechanizm, który ujednolici prace nad tymi szarlatanami,
...
<Section>
  ...
  <M>
    ...
    identyczne - mają taką samą sygnature.{" "}
  </M>
  <Snippet>
...
`}</Snippet>
  <M>
    Wiadomość błędu nie koniecznie może naprowadzić na przyczynę błędu. Jest to
...
<Section>
  ...
  <M>
    ...
    wspierane <B>storage</B> oraz dodajmy szkielet funkcji <B>storage</B>.
  </M>
  <Snippet>
    {`
...
<M>Następnie dodamy implementację metody do odczytu pojedyńczej wartości: <B>get</B>. Zwróć uwagę na rzutowanie, które jest niezbędne - metoda <B>JSON.parse</B> nie pozwala na przekazanie generycznego parametru.</M>
...
<M>
  ...
  wyjątków i odczyt pojedyńczej wartości oraz wielu wartości działa prawidłowo.
</M>
...
<M>
  ...
  funkcja, która odczyta "najnowsze" wartości. Jeżeli była by to zmienna - to
...
<Summary label="Podsumowanie">
  ...
  <M>
    Warto wspomnieć, że wprowadzanie abstrakcji takiej jak tak nie zawsze ma
