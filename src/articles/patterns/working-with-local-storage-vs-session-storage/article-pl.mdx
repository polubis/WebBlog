...
`}
  ...
  <Code mode="static" lang="ts">{`const value = localStorage.getItem('literó,wka')`}</Code>
  <M>
...
blogAPI.get()`}
  ...
  <Code mode="static" lang="ts" description="Tak będzie wyglądała inicjalizacja">{`
...
`}</Code>
  ...
  <Code mode="static" lang="ts" description="A tak będą wyglądać nasze metody">{`
...
`}
  ...
  <Code mode="static" lang="ts" description="W tym teście sprawdzamy, czy otrzymaliśmy odpowiedni wyjątek w przypadku wykorzystania mechanizmu po stronie serwera">{`
import { storage } from '../storage'
...
`}</Code>
  <Code mode="static" lang="ts" description="Tu sprawdzamy, czy mechanizm odczytu kluczy działa prawidłowo">
  ...
  <Code mode="static" lang="ts" description="Testowana metoda ma za zadanie ustawić kilka wartości">{`
it('allows to set several values', () => {
  ...
  <Code mode="static" lang="ts" description="Sprawdzamy, czy zwracane są tylko wartości, które zostały ustawione">{`
it('allows to get all values', () => {
  ...
  <Code mode="static" lang="ts" description="W tym teście sprawdzamy, jak zachowuje się nasz mechanizm i symulujemy prawdziwy scenariusz użycia">{`
